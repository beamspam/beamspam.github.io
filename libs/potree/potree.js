...
Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher{
	...
	constructor(domElement, args){
		...
		{// set defaults
			...
			this.setNavigationMode(Potree.FirstPersonControls);
...
function initSceneList(){
	...
	var ellabel = $('<h2>');
	ellabel.text("Uncalibrated")
	scenelist.append(ellabel)
	var elbutton = $('<button>').text("Toggle Point Cloud").click(function() {
		for (var i = 0; i < viewer.scene.pointclouds.length; i++) {
			var pointcloud = viewer.scene.pointclouds[i];
			pointcloud.visible = !pointcloud.visible;
			if (i === 0 && pointcloud.visible) {
                ellabel.text("Uncalibrated")
			} else if (i === 1 && pointcloud.visible) {
				ellabel.text("Calibrated")
			}

		}
	});
	scenelist.append(elbutton);

	var elimgdiv = $('<div>');
	var elimg = $('<img>').attr("src", "Legend.png");
	elimgdiv.append(elimg);
	scenelist.append(elimgdiv);
